...
export default function DriverOnboarding({
  ...
  const handleSubmitProfile = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/driver/profile`,
        {
          method: "POST",
          headers: {
  ...
  const handleSubmitVehicle = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/driver/vehicles`,
        {
          method: "POST",
          headers: {
            "Content-Type": process.env.NEXT_PUBLIC_CONTENT_TYPE!,
      ...
      toast.success("Onboarding complete! Your account is under review.");
      setPage("home");
    } catch (err: any) {
      setError(err.message);
    } finally {
    ...
    return (
      <div className="text-center py-16">
        <p className="text-lg text-red-600">
          Please log in to access driver onboarding.
        </p>
      </div>
    );
  }
  ...
  return (
    <section className="max-w-md mx-auto mt-12">
      <h2 className="text-2xl font-bold text-black-800 mb-6 text-center">
        Driver Onboarding
      </h2>
